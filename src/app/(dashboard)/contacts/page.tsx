...
export default function ContactsPage() {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      ...
      const enriched: ContactWithTags[] = contactRows.map((c: any) => ({
        ...c,
        tags: (c.tag_ids || []).map((tid: string) => tagsMap[tid]).filter(Boolean),
      }));
    ...
    setLoading(false);
  }, [page, search, selectedTagIds, tagsMap, t]);
  ...
  async function openEditForm(contact: Contact) {
    ...
    setEditContactTags(tagIds.map((id: string) => ({ contact_id: contact.id, tag_id: id })));
    setFormOpen(true);
  }
